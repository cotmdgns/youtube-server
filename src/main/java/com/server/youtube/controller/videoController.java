...
import com.server.youtube.domian.QVideo;
import com.server.youtube.domian.VideoDTO;
...
public class VideoController {
    ...
    // 비디오 전체 목록 보기
    @GetMapping("/video")
    public ResponseEntity allVideo(@RequestParam(name="page",defaultValue = "1")int page,@RequestParam(name="keyword",required=false)String keyword){
        ...
        if (keyword !=null && !keyword.equals("")) {
            ...
            // 만들어진 조건은 where문에 and나 or 같은 키워드와 결함해서 추가
    ...
    public ResponseEntity create(VideoDTO dto) throws IOException {
        ...
        File videoFile = new File(path + "video" + File.separator + videoName); // + File.separator = / 역활함
